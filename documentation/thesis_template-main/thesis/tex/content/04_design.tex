...
\[X(0) = C_1 \cdot e^{0 \cdot \sqrt{w}}+ C_2 \cdot e^{-0 \cdot \sqrt{w}} = 0 \]
...
\[X(0) = C_1 \cdot e^{0 \cdot \sqrt{w}} + C_2 \cdot e^{-0 \cdot \sqrt{w}} = C_1 \cdot 1 + C_2 \cdot 1 = 
C_1 + C_2 = 0 => C_1 = -C_2 => |C_1| = |C_2| = c\]

\[X(x) = C \cdot e^{x \cdot \sqrt{w}} - C\cdot e^{-x \cdot \sqrt{w}} = C( e^{x \cdot \sqrt{w}} - e^{-x \cdot \sqrt{w}}) \]

\[X(0) = C_1 \cdot e^{0\sqrt{\omega}} + C_2 \cdot e^{-0\cdot \sqrt{\omega}}\]
\[X(0) = C_1 \cdot 1 + C_2 \cdot 1\]
\[C_1 + C_2 = 0\]
\[C_1 = -C_2\]
\[C_1| = |C_2| = C\]
\[X(x) = C \cdot e^{x\sqrt{\omega}} - C \cdot e^{-x\cdot \sqrt{\omega}}\]


\[X(\pi) = 0 = C \cdot ( e^{\pi \cdot \sqrt{w}} - e^{-\pi \cdot \sqrt{w}} )  C \cdot \neq 0\] 
\[\Rightarrow e^{\pi \cdot \sqrt{w}} - e^{-\pi \cdot \sqrt{w}} = 0\] 
\[\Rightarrow e^{\pi \cdot \sqrt{w}} = e^{-\pi \cdot \sqrt{w}}\]
\[\Rightarrow e^{2 \cdot \pi \cdot \sqrt{w}} = 1\]

Wegen Gausscher Zahlenebene \[e^{2 \cdot \pi \cdot \sqrt{w}} = e^{i \cdot 2 \cdot \pi \cdot \sqrt{w}}\]    korregieren !!!!!

\[\Rightarrow 2\cdot \pi \cdot \sqrt{w} = i \cdot 2 \cdot \pi \cdot k,\qquad k \in \mathbb{Z} \setminus \{0\}
\]
\[\Rightarrow \sqrt{w} = i \cdot k, \qquad k \in \mathbb{Z} \setminus \{0\}
\]
\[\Rightarrow w = -k^{2}, \qquad k \in \mathbb{Z} \setminus \{0\}
\]


Einsetzen in  $\lambda = a \pm i b$  
\[X(x) = e^{a\cdot x} + (C_1 \cdot cos(bx) + C_2 \cdot sin(bx))\]

\[X(x) = C_1 \cdot cos(kx) + C_2 \cdot sin(kx) = C_1 \cdot \cos(kx) + C_2 \cdot \sin(kx) \]
\[X(0) = C_1 \cdot cos(0) + C_2 \cdot sin(0) = C_1 \cdot 1 + C_2 \cdot 0 = C1 = 0 \]
\[X(0) = C_1 \cdot \cos(0) + C_2 \cdot \sin(0) = C_1 \cdot 1 + C_2 \cdot 0 = C_1 = 0 \]

\[X(x) = C_2 \cdot sin(kx)
 X(x) = C_2 \cdot \sin(kx) \]
\[X(\pi) = C_2 \cdot sin(k \cdot \pi) = 0\]
\[sin(k \cdot \pi) = 0, \qquad C_2 \neq 0,  \qquad k \in \mathbb{Z} \setminus \{0\} \]    ???? richtig 
\[
X(\pi) = C_2 \cdot \sin(k \cdot \pi) = 0, \qquad C_2 \neq 0, \qquad k \in \mathbb{Z} \setminus \{0\}
\]

Zusammenführung der Lösung mit Randbedingungen:
\[U(x,t) = C_2\cdot sin(k \cdot x) \cdot C_3 \cdot e^{-a^2 \cdot k^2 \cdot t}, \qquad Ck = C_2 \cap C_3 = \text{Konst.} \]
...
\[C_k = 2\cdot \pi \cdot \text{Integral von 0 bis } \pi \text{ von } U(x,0)*sin(kx) dx\]
\[C_k = 2 \cdot \pi \cdot \text{Integral von 0 bis } \pi \text{ von }1*sin(kx) dx\]
\[
C_k = \frac{2}{\pi} \int_{0}^{\pi} U(x,0)\sin(k \cdot x)\,\mathrm{d}x\]
\[
C_k = \frac{2}{\pi} \int_{0}^{\pi} 1 \cdot \sin(k \cdot x)\,\mathrm{d}x
\]
\[ C_k = \frac{2}{\pi} \int_{0}^{\pi} \left( -\frac{1}{k} \cdot \cos(k \cdot x) \right) \,\mathrm{d}x \]
...
\[
U(x,t) = \frac{4}{\pi \cdot k}\cdot \,\sin(k \cdot x)\cdot \,e^{-a^2 \cdot k^2 \cdot t}
\]
