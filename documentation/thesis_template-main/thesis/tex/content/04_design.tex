...
\[\frac{ X_{xx}(x)}{ X(x)}  = \frac{T_t(t)}{ a^2 * T(t) } = q \qquad   q = Konst.\] (!!!mathematischen lambda trick erklären!!!!))
...
\[U(x,t)=(C_1e^{x*\sqrt{w}} + C_2e^{-x*\sqrt{w}})*C_3e^{a^2 * w}\]

Einsetzen der Randbedingungen \[U(0,t) =  0\] 

in, Ortsteil der Gleichungen 
\[X(0) = C_1e^{0*\sqrt{w}}+ C_2*e^{-0*\sqrt{w}} = 0 \]
...
\[X(0) = C_1e^{0*\sqrt{w}} + C_2e^{-0*\sqrt{w}} = C_1*1 + C_2*1 = 
C_1 + C_2 = 0 => C_1 = -C_2 => |C_1| = |C_2| = c\]

\[X(x) = Ce^{x*\sqrt{w}} - Ce^{-x*\sqrt{w}} = C( e^{x*\sqrt{w}} - e^{-x*\sqrt{w}}) \]
...
\[X(x) = e^{a*x} + (C_1*cos(bx) + C_2*sin(bx))\]

\[X(x) = C_1cos(kx) + C_2sin(kx) = C_1 \cos(kx) + C_2 \sin(kx) \]
\[X(0) = C_1cos(0) + C_2sin(0) = C_1*1 + C_2*0 = C1 = 0 \]
\[X(0) = C_1 \cos(0) + C_2 \sin(0) = C_1 \cdot 1 + C_2 \cdot 0 = C_1 = 0 \]

\[X(x) = C_2sin(kx)
 X(x) = C_2 \sin(kx) \]
\[X(\pi) = C_2sin(k*\pi) = 0\]
\[sin(k*\pi) = 0 \qquad C_2 \neq 0  \qquad k \in \mathbb{Z} \setminus \{0\} \]    ???? richtig 
\[
X(\pi) = C_2 \sin(k\pi) = 0 \qquad C_2 \neq 0 \qquad k \in \mathbb{Z} \setminus \{0\}
\]

Zusammenführung der Lösung mit Randbedingungen:
\[U(x,t) = C_2sin(kx)*C_3e^{-a^2*k^2*t}\ \qquad Ck = C_2C_3 = \text{konst.} \]
\[U(x,t) = C_ksin(kx)*e^{-a^2*k^2*t}\]         
...
\[C_k = 2\pi * \text{Integral von 0 bis } \pi \text{ von } U(x,0)*sin(kx) dx\]
\[C_k = 2\pi * \text{Integral von 0 bis } \pi \text{ von }1*sin(kx) dx\]
\[
