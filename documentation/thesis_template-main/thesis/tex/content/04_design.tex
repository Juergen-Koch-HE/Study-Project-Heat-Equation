...
Es wurde der Produktansatz gewählt, um die Wärmeleitungsgleichung aufzustellen \cite[S.~105-107]{langemann2018so}. Beginnend mit der Separation der Variablen, um den Ort $X(x)$ und die Zeit $T(t)$ zu trennen. Dadurch wird die Gleichung in zwei Bereiche aufgeteilt und sie können nach der jeweiligen Unbekannten aufgelöst werden. 
Wärmeleitungsgleichung: \[U_t= a^2 \cdot U_{xx}\]
...
Einsetzen in die Wärmeleitungsgleichung. 
\[X(x) \cdot T_t(t) = a^2 \cdot X_{xx}(x) \cdot T(t)\]

Durch die Separation der Variablen entstehen zwei unabhängige Gleichungsteile, nach dem Ort $x$ und der Zeit $t$. Dadurch könnte man den Ort verändern und würde keinen Einfluss auf die Zeit nehmen und umgekehrt. Das bedeutet mathematisch, dass man beide Gleichungen getrennt betrachten kann und damit die Gleichungen für alle $x$ und $t$ erfüllt sind, Konstanten eingesetzt werden müssen. In diesem Fall die Konstante $w$. Das bringt den Vorteil, dass die Wärmeleitungsgleichung, welche eine partielle Differenzialgleichung ist, in zwei gewöhnliche Differenzialgleichungen geteilt wird. Die einmal nach dem Ort und einmal nach der Zeit getrennt sind. %(Mathematik ..., xxxx, S.105-107)
