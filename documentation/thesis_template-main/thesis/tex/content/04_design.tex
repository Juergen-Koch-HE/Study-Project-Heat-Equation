...
(Koch & Stempfle, xxxx, S.501) Auflage 18

\subsubsubsection{Characteristische Gleichung}
Das Prinzip der Characteristischen Gleichung wird beim lösern linearere homogener Differentialgleichungen angwendet und erleichter das auflösen der Gleichung. Dabei wird der Exponentialansatz dazu genutzt: $Y(x) = e^{\lambda*x}$

Dabei wird also beim lösen der Gleichung die Ableitung $Y(x)^k$ jeweils durch $\lambda^k$ ersetzt. Dabei ist darauf zu achten das $Y(x)^0 = \lambda^0 = 1 $ und $Y(x)^1 =\lambda$ ist.
...
\[ 
Y_{xx}(x)+2*Y_{x}+Y(x)= 0 \Rightarrow\ \lambda^2+\lambda+1 = 0
\]

\[
\lambda_{1,2}=\frac{-2\pm\sqrt{2^2-4\cdot1\cdot1}}{2\cdot1}
...
Aus den lösungen der einfachen oder mehrfache reele Eigenwerte werden Eigenwertfunktionen die Werte von $\lambda_1$ bis $\lambda_n$ werden in die Eigenwertfunktion $C_1*e^{\lambda_1*x}+ ... + C_n*e^{\lambda_n*x}$
